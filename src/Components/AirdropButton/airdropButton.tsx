...
const AirdropButton: React.FC<Props> = (props: Props) => {
        ...
        const getAirdrop = async (network: Network, userAddress: string, provider: ethers.providers.Web3Provider) => {
            ...
            if (airdrop) {
                ...
                Object.values(airdrop).map((a) => {
                    const hasAirdrop = Object.keys(a.accounts).find(
                        (x) => x.toLowerCase() === userAddress.toLowerCase(),
                    );
                    if (hasAirdrop) {
                        const merkle = new MerkleTree(
                            Object.entries(a.accounts).map(([address, tokens]) =>
                                generateLeaf(ethers.utils.getAddress(address), tokens),
                            ),
                            keccak256,
                            { sortPairs: true },
                        );
    ...
    return props.airdrops.length > 0 && props.hasClaimed ? (
        <div
            className={`airdrop-button ${!props.spinner ? 'airdrop-button-hover' : 'airdrop-button-spinner'}`}
            onClick={() => (props.spinner ? null : handleOpenAirdropMenu())}
        >
            <div className="airdrop-button-content">
                {totalAmount.length > 0 ? (
                    <>
                        <span className="airdrop-name">
                            <StarBpro active={true} backstop={false} />
                        </span>
                        <span className="airdrop-amount">
                            {totalAmount.map((x, index) => {
                                return (
                                    <div key={index}>
                                        {x.value.toRound(2, true, true)} {x.symbol}
                                    </div>
                                );
                            })}
                        </span>
