...
const BorrowMarketDialog: React.FC<Props> = (props: Props) => {
        ...
        const handleBorrowAmountChange = () => {
            ...
            if (props.generalData && props.market) {
                const value =
                    ...
                    +BigNumber.parseValue(borrowInput).toString() * +props.market?.underlying.price.toString();
    ...
    useEffect(() => {
        const handleRepayAmountChange = () => {
            ...
            if (isNaN(+repayInput)) {
                ...
                !isFullRepay &&
                props.market &&
                BigNumber.parseValue(repayInput).gt(props.market?.borrowBalanceInTokenUnit)
            ) {
                ...
                const approve = BigNumber.parseValue(
                    (+repayInput - +props.market?.underlying.allowance.toString()).noExponents(),
                ).toRound(4);
    ...
    const handleMaxBorrow = async () => {
        if (props.generalData && props.market) {
            const balance =
                (+props.generalData.totalBorrowLimit.toString() - +props.generalData.totalBorrowBalance.toString()) /
            ...
            if (props.generalData && props.market) {
                const value =
                    +props.generalData.totalBorrowBalance.toString() +
                    +BigNumber.parseValue(borrow.toString()).toString() * +props.market?.underlying.price.toString();
                pValue = BigNumber.parseValue(
                    (+props.generalData.totalBorrowLimit.toString() > 0
                        ? (+value / +props.generalData.totalBorrowLimit.toString()) * 100
                        : 0
                ...
                if (+pValue.toRound(2) >= 50.01) {
                    borrow =
                        (0.5 * +props.generalData.totalBorrowLimit.toString() -
                            +props.generalData.totalBorrowBalance.toString()) /
                        +props.market?.underlying.price.toString();
                }
            }
                    ...
                    <TabContent>
                        <TabContentItem open={props.open} tabId={1} tabChange={tabChange}>
                            <TextBox
                                ...
                                disabled={borrowDisabled || (props.market ? props.market.borrowPaused : false)}
                                value={borrowInput}
                                ...
                                buttonDisabled={
                                    props.generalData &&
                                    +props.generalData?.totalBorrowLimitUsedPercent.toRound(2) >= 50.01
                                        ? true
                                        : false
                                }
                            ...
                            <MarketDialogItem
                                title={'You Borrowed'}
                                value={`${props.market?.borrowBalanceInTokenUnit?.toRound(4, true)} ${
                                    props.market?.underlying.symbol
                                }`}
                            />
                            ...
                            {props.market && props.market.borrowPaused ? (
                                ...
                                <MarketDialogButton
                                    disabled={
                                        !borrowInput || borrowValidation || props.market?.borrowSpinner ? true : false
                                    }
                                    onClick={() => {
                                        props.market
                                            ? props.handleBorrow(props.market?.underlying.symbol, borrowInput)
                                            : null;
                                    }}
                                >
                        ...
                        <TabContentItem open={props.open} tabId={2} tabChange={tabChange}>
                            ...
                            +props.market.underlying.allowance.toString() >=
                                (repayInput.trim() === '' || isNaN(+repayInput) || isNaN(parseFloat(repayInput))
                                    ? 0
                                    : +repayInput) ? (
                                <MarketDialogButton
                                    disabled={
                                        !repayInput || repayValidation || props.market?.repaySpinner ? true : false
                                    }
                                    onClick={() => {
                                        props.market
                                            ? props.handleRepay(
