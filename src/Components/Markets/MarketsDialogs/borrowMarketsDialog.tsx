...
const BorrowMarketDialog: React.FC<Props> = (props: Props) => {
  ...
  useEffect(() => {
    const handleBorrowAmountChange = () => {
      ...
      if (props.generalData && props.market) {
        const value =
          +props.generalData.totalBorrowBalance.toString() + +BigNumber.parseValue(borrowInput).toString() * +props.market?.underlying.price.toString();
        pValue = BigNumber.parseValue(
          (+props.generalData.totalBorrowLimit.toString() > 0 ? (+value / +props.generalData.totalBorrowLimit.toString()) * 100 : 0).toFixed(18),
        );
      }
  ...
  const handleMaxRepay = async () => {
    const maxAffordable = props.market ? await props.getMaxAmount(props.market, 'repay') : BigNumber.from('0');
    const fullRepayAmount = props.market ? await props.getMaxRepayAmount(props.market) : BigNumber.from('0');

    const isFull = maxAffordable.gteSafe(fullRepayAmount);
    setIsFullRepay(isFull);

    //min(debt * (1 + borrowAPY/365), walletbalance - gasAllowance)
    setRepayInput(BigNumber.minimum(maxAffordable, fullRepayAmount).toString());
  };

  const handleMaxBorrow = async () => {
    if (props.generalData && props.market) {
      const balance =
        (+props.generalData.totalBorrowLimit.toString() - +props.generalData.totalBorrowBalance.toString()) / +props.market.underlying.price.toString() / 2;
      const amount = +props.market.cash.toString() / 2;
      let borrow = 0;
      if (balance > amount) borrow = amount;
      else borrow = balance;

      let pValue = BigNumber.from('0');
      if (props.generalData && props.market) {
        const value =
          +props.generalData.totalBorrowBalance.toString() + +BigNumber.parseValue(borrow.toString()).toString() * +props.market?.underlying.price.toString();
        pValue = BigNumber.parseValue(
          (+props.generalData.totalBorrowLimit.toString() > 0 ? (+value / +props.generalData.totalBorrowLimit.toString()) * 100 : 0).toFixed(18),
        );
        if (+pValue.toRound(2) >= 50.01) {
          borrow =
            (0.5 * +props.generalData.totalBorrowLimit.toString() - +props.generalData.totalBorrowBalance.toString()) /
            +props.market?.underlying.price.toString();
        }
      }
      ...
      <div ref={ref} className="supply-box">
        ...
        <div className="dialog-title">
          {props.market?.underlying.symbol && (
            <img className="rounded-circle" style={{ width: '30px', margin: '0px 10px 0px 0px' }} src={props.market?.underlying.logo} alt="" />
          )}
          {`${props.market?.underlying.symbol}`}
        </div>
        <Tab>
          ...
          <TabContent>
            <TabContentItem open={props.open} tabId={1} tabChange={tabChange}>
              <TextBox
                ...
                value={borrowInput}
                setInput={setBorrowInput}
                validation={borrowValidation}
                button={'Safe Max'}
                buttonTooltip="50% of borrow limit"
                buttonDisabled={props.generalData && +props.generalData?.totalBorrowLimitUsedPercent.toRound(2) >= 50.01 ? true : false}
                onClick={() => handleMaxBorrow()}
              />
              <MarketDialogItem
                title={'You Borrowed'}
                value={`${props.market?.borrowBalanceInTokenUnit?.toRound(4, true)} ${props.market?.underlying.symbol}`}
              />
              <BorrowRateSection market={props.market} darkMode={props.darkMode} />
              <BorrowLimitSection2 generalData={props.generalData} market={props.market} borrowAmount={borrowInput} repayAmount={'0'} />
              <DialogMarketInfoSection market={props.market} collateralFactorText={'Liquidation Threshold'} />
              <MarketDialogButton
                disabled={!borrowInput || borrowValidation || props.market?.borrowSpinner ? true : false}
                onClick={() => {
                  props.market ? props.handleBorrow(props.market?.underlying.symbol, borrowInput) : null;
                }}
              >
            ...
            <TabContentItem open={props.open} tabId={2} tabChange={tabChange}>
              <TextBox
                ...
                value={repayInput}
                setInput={setRepayInput}
                validation={repayValidation}
                button={'Max'}
                onClick={() => handleMaxRepay()}
                onChange={() => setIsFullRepay(false)}
              />
              <MarketDialogItem
                title={'Wallet Ballance'}
                value={`${props.market?.underlying.walletBalance?.toRound(4, true)} ${props.market?.underlying.symbol}`}
              />
              <BorrowRateSection market={props.market} darkMode={props.darkMode} />
              <BorrowLimitSection2 generalData={props.generalData} market={props.market} borrowAmount={'0'} repayAmount={repayInput} />
              <DialogMarketInfoSection market={props.market} collateralFactorText={'Liquidation Threshold'} />

              {props.market &&
              props.market?.underlying.allowance?.gt(BigNumber.from('0')) &&
              +props.market.underlying.allowance.toString() >=
                (repayInput.trim() === '' || isNaN(+repayInput) || isNaN(parseFloat(repayInput)) ? 0 : +repayInput) ? (
                <MarketDialogButton
                  disabled={!repayInput || repayValidation || props.market?.repaySpinner ? true : false}
                  onClick={() => {
                    props.market ? props.handleRepay(props.market?.underlying.symbol, repayInput, isFullRepay) : null;
                  }}
                >
                ...
                <MarketDialogButton
                  disabled={props.market && props.market?.repaySpinner ? true : false}
                  onClick={() => {
                    props.market ? props.handleEnable(props.market?.underlying.symbol, true) : null;
                  }}
                >
