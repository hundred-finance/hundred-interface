...
const SupplyMarketDialog: React.FC<Props> = (props: Props) => {
    ...
    const CloseDialog = () => {
        if (props.spinnerVisible) return;
        setSupplyInput('');
        setWithdrawInput('');
    ...
    useEffect(() => {
        const handleSupplyAmountChange = () => {
            ...
            if (isNaN(+supplyInput) || isNaN(parseFloat(supplyInput))) {
                ...
                const approve = BigNumber.parseValue(
                    (+supplyInput - +props.market?.underlying.allowance.toString()).noExponents(),
                ).toRound(4);
            ...
            setNewBorrowLimit1(
                props.generalData && props.market
                    ? props?.generalData.totalBorrowLimit?.addSafe(
                          props.market?.isEnterMarket
                              ? BigNumber.parseValue(supplyInput !== '' ? supplyInput : '0')
                                    .mulSafe(props.market?.underlying.price)
                                    .mulSafe(props.market?.collateralFactor)
                              : BigNumber.from(0),
                      )
                    : BigNumber.from(0),
            );
    ...
    useEffect(() => {
        const handleWithdrawAmountChange = () => {
                ...
                if (withdrawMax && props.market) {
                    if (
                        BigNumber.parseValueSafe(
                            props.market?.supplyBalanceInTokenUnit.toString(),
            ...
            setNewBorrowLimit2(
                props.market && props.generalData
                    ? props.generalData.totalBorrowLimit?.subSafe(
                          props.market?.isEnterMarket
    ...
    useEffect(() => {
        const handleStakeAmountChange = () => {
            ...
            if (props.market && props.generalData) {
                ...
                const stake =
                    props.market.isEnterMarket && stakeInput.trim() !== ''
                        ? +convertLpAmountToUnderlying(stakeInput, props.market)
                        : 0;
    ...
    useEffect(() => {
        const handlebackstopWithdrawChange = () => {
            ...
            if (isNaN(+backstopWithdrawInput) || isNaN(parseFloat(backstopWithdrawInput))) {
                ...
                +backstopWithdrawInput > +props.market?.backstop?.userBalance
            ) {
                setBackstopWithdraw('Withdraw');
                setBackstopWithdrawValidation('Amount must be <= balance');
            } else {
                if (props.market?.backstop) {
                    const widthdrawUsd = BigNumber.parseValue(
                        (+backstopWithdrawInput * props.market.backstop.sharePrice.toNumeral()).noExponents(),
                    );
                    if (widthdrawUsd.toNumeral() > 0) {
                        ...
                        else setBackstopWithdraw(`Withdraw (>$${widthdrawUsd.toRound(2, true, true)})`);
                ...
                ? props.generalData.totalBorrowLimit?.subSafe(
                      props.market?.isEnterMarket
                          ? BigNumber.parseValue(
                                stakeInput.trim() !== '' ? convertLpAmountToUnderlying(stakeInput, props.market) : '0',
                            )
                                .mulSafe(props.market?.underlying.price)
                                .mulSafe(props.market?.collateralFactor)
                          : BigNumber.from(0),
    ...
    const getMaxAmount = async (deposit?: boolean): Promise<void> => {
        ...
        if (deposit) setDepositInput(amount.toString());
        else setSupplyInput(amount.toString());
    };
        ...
        props.market
            ? setWithdrawInput(
                  BigNumber.minimum(
                      ...
                      BigNumber.parseValueSafe(props.market?.cash.toString(), props.market.underlying.decimals),
                  ).toString(),
              )
            : setWithdrawInput('0');
    };
    ...
    const getMaxBackstopWithdraw = (): void => {
        props.market && props.market.backstop
            ? setBackstopWithdrawInput(props.market.backstop.userBalance.toString())
    ...
    const setUnstakeRatio = (ratio: number) => {
        setUnstakeInput(
            BigNumber.from(
                Math.floor(formatBalance(props.gaugeV4?.userStakeBalance).toNumber() * ratio),
    ...
    const setStakeRatio = (ratio: number) => {
        setStakeInput(
            BigNumber.from(
                Math.floor(formatBalance(props.gaugeV4?.userLpBalance).toNumber() * ratio),
                ...
                <Tab>
                    ...
                    <TabContent>
                        <TabContentItem open={props.open} tabId={1} tabChange={tabChange}>
                            <TextBox
                                placeholder={`0 ${props.market?.underlying.symbol}`}
                                disabled={supplyDisabled || (props.market ? props.market?.mintPaused : false)}
                                value={supplyInput}
                            ...
                            <MarketDialogItem
                                ...
                                value={`${props.market?.underlying.walletBalance?.toRound(4, true)} ${
                                    props.market?.underlying.symbol
                                }`}
                            />
                            <SupplyRateSection
                            ...
                            {props.market && props.market.mintPaused ? (
                              ...
                              +props.market.underlying.allowance.toString() > 0 &&
                              +props.market.underlying.allowance.toString() >=
                                ...
                                <MarketDialogButton
                                    ...
                                    onClick={() => {
                                        props.market
                                            ? props.handleSupply(props.market?.underlying.symbol, supplyInput)
                                            : null;
                                    }}
                                >
                                ...
                                <MarketDialogButton
                                    disabled={!props.market || (props.market && props.market?.supplySpinner)}
                                    onClick={() => {
                                        props.market
                                            ? props.handleEnable(props.market?.underlying.symbol, false)
                                            : null;
                                    }}
                                >
                                    {props.market?.supplySpinner ? (
                                        ...
                                        `Approve ${props.market?.underlying.symbol}`
                                    )}
                                </MarketDialogButton>
                            )}
                        </TabContentItem>
                        {props.gaugeV4 && props.market ? (
                            <TabContentItem open={props.open} tabId={2} tabChange={tabChange}>
                                <MarketDialogItem
                                    ...
                                    value={`${formatBalance(
                                        BigNumber.from(
                                            props.gaugeV4?.userStakedTokenBalance,
                                            props.market.underlying.decimals,
                                        ).mul(props.market.exchangeRate),
                                    ).toFixed(4)} ${props.market?.underlying.symbol}`}
                                />
                                <MarketDialogItem
                                    title={'Claimable'}
                                    value={`${formatBalance(props.gaugeV4?.userClaimableHnd).toFixed(4)} HND`}
                                />
                                <MarketDialogItem title={'APR'} value={stakingApr(props.market, props.gaugeV4)} />
                                ...
                                <div className="native-asset-amount">
                                    ...
                                    <div className="amount-select">
                                        ...
                                        <div onClick={() => setStakeRatio(0.5)}>50%</div>
                                ...
                                <div className="input-button-group">
                                    ...
                                    +props.gaugeV4.userAllowance.toString().toString() >=
                                        (stakeInput.trim() === '' || isNaN(+stakeInput) || isNaN(parseFloat(stakeInput))
                                            ? 0
                                            : +stakeInput) ? (
                                        <MarketDialogButton
                                            disabled={
                                                ...
                                                (+newBorrowLimit3.toString() > 0
                                                    ...
                                                    : (+props.generalData?.totalBorrowBalance.toString() /
                                                          +newBorrowLimit3.toString()) *
                                                      -1 *
                                                      100) > +props.generalData.totalBorrowLimitUsedPercent
                                                    ? true
                                                    : false)
                                            }
                                            onClick={() =>
                                ...
                                <div className="native-asset-amount">
                                    <span>
                                        {convertGaugeLpAmountToUnderlying(
                                            unstakeInput,
                                            props?.gaugeV4.gaugeTokenDecimals,
                                            props.market,
                                        )}{' '}
                                        {props.market?.underlying.symbol}
                                    </span>
                                    <div className="amount-select">
                                        ...
                                        <div onClick={() => setUnstakeRatio(0.5)}>50%</div>
                                ...
                                <MarketDialogButton
                                    disabled={
                                        props?.gaugeV4?.userClaimableHnd === undefined ||
                                        props?.gaugeV4?.userClaimableHnd?.eq(BigNumber.from(0))
                                    }
                        ...
                        <TabContentItem
                            open={props.open}
                            tabId={props.gaugeV4 && props.market ? 3 : 2}
                            tabChange={tabChange}
                        >
                            <TextBox
                                placeholder={`0 ${props.market?.underlying.symbol}`}
                            ...
                            <MarketDialogItem
                                title={'You Supplied'}
                                value={`${props.market?.supplyBalanceInTokenUnit?.toFixed(4)} ${
                                    props.market?.underlying.symbol
                                }`}
                            />
                            <SupplyRateSection
                        ...
                        {props.market?.backstop ? (
                            <TabContentItem
                                open={props.open}
                                tabId={props.gaugeV4 && props.market ? 4 : 3}
                                tabChange={tabChange}
                            >
                                <MarketDialogItem
                                    ...
                                    className="dialog-section-no-bottom-gap"
                                />
                                ...
                                <TextBox
                                    ...
                                    button={'Max'}
                                    onClick={() => getMaxAmount(true)}
                                    validationCollapse={true}
                                />
                                {props.market?.backstop.allowance?.gt(BigNumber.from(0)) &&
                                props.market?.backstop.allowance?.gte(
                                    depositInput.trim() === '' || isNaN(+depositInput)
                                        ? BigNumber.from('0')
                                    ...
                                    <MarketDialogButton
                                        disabled={
                                            !props.market || (props.market && props.market?.backstopDepositSpinner)
                                        }
                                        onClick={() => {
                                            props.market
                                                ? props.handleApproveBackstop(props.market?.underlying.symbol)
                                                : null;
                                        }}
                                    >
                                        {props.market?.backstopDepositSpinner ? (
                                ...
                                <TextBox
                                    placeholder={`0 ${props.market.backstop.symbol}`}
                                    disabled={backstopWithdrawDisabled}
                                    value={backstopWithdrawInput}
                                    setInput={setBackstopWithdrawInput}
                                    validation={backstopWithdrawValidation}
                                    button={'Max'}
                                    onClick={() => getMaxBackstopWithdraw()}
                                    validationCollapse={true}
                                />
                                <MarketDialogButton
                                    ...
                                    disabled={
                                        backstopWithdrawInput === '' ||
                                        backstopWithdrawValidation !== '' ||
                                        isNaN(+backstopWithdrawInput) ||
                                        props.market?.backstopWithdrawSpinner ||
                                        BigNumber.parseValue(backstopWithdrawInput).gt(
                                            props.market.backstop.userBalance,
                                        )
                                            ? true
                                            : false
                                    }
                                ...
                                >
                                    {props.market && props.market.backstopWithdrawSpinner ? (
                                        <Spinner size={'20px'} />
                                    ) : (
                                        backstopWithdraw
                                    )}
                                ...
                                {+props.market.backstop.pendingHundred.toString() > 0 ? (
                                    <MarketDialogButton
                                        className="backstop-dialog-button"
                                        disabled={props.market?.backstopClaimSpinner ? true : false}
                                        onClick={() => {
                                            props.market
                                                ? props.handleBackstopClaim(props.market?.underlying.symbol)
                                                : null;
                                        }}
                                    >
                                        {props.market && props.market.backstopClaimSpinner ? (
                                            <Spinner size={'20px'} />
                                        ) : (
                                            `Claim ${props.market.backstop.pendingHundred.toRound(4, true, true)} HND`
                                        )}
                                    </MarketDialogButton>
                                ) : (
                                    <></>
