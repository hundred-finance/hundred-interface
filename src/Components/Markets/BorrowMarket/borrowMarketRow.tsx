...
const BorrowMarketRow: React.FC<Props> = (props: Props) => {
    return (
        <tr onClick={() => (props.details ? props.borrowMarketDialog(props.details) : null)}>
            ...
            <td>
                {props.details
                    ? +props.details.borrowBalanceInTokenUnit.toString() > 0.001 ||
                      +props.details.borrowBalanceInTokenUnit.toString() === 0
                        ? +props.details.borrowBalanceInTokenUnit.toFixed(4)
                        : '<0.001'
                    : '0'}
            </td>

            <td>{props.details ? +props.details.underlying.walletBalance.toFixed(4).toString() : '0'}</td>
            <td>
