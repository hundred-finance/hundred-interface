    ...
    return (
        <div className="airdrop-menu">
            {props.airdrops.filter((x) => !x.hasClaimed).length > 0 ? (
                <>
                    <div className="airdrop-menu-label">Unclaimed Airdrop</div>
                                    ...
                                    <div key={index} className="airdrop-menu-item">
                                        <div className="airdrop-menu-item-values">
                                            {a.amount.map((am, index2) => (
                                                <div key={`a-${index2}`}>
                                                    {am.value.toRound(2, true, true)} {am.symbol}
                                                </div>
                                            ))}
                                        </div>
                                        <MoneyBagIcon
                    ...
                    <div className="airdrop-menu-item">
                        <button
                            className={`${
                                props.spinner ? 'airdrop-menu-item-button-disabled' : 'airdrop-menu-item-button'
                            }`}
                            onClick={() => (props.spinner ? null : handleClaimAll())}
                        >
            ...
            {props.airdrops.filter((x) => x.hasClaimed).length > 0 ? (
                <>
                    <div className="airdrop-menu-label">Airdrop History</div>
                    {
