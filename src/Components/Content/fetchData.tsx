        ...
        while (res.length) {
            ...
            const token = backstop
                ? backstop.collaterals
                    ? res.splice(0, 32 + backstop.collaterals.length * 3)
                    : res.splice(0, 33)
                : res.splice(0, 21);
            tokens.push(
                await getTokenData(
    ...
    const tokensInfo = await Promise.all(
        tokens.map(async (t) => {
            const tokenInfo = await getCtokenInfo(
                ...
                hndPrice,
                blockNum,
                gaugesData.find((g) => g.generalData.lpToken.toLowerCase() === t.tokenAddress.toLowerCase()),
            );
            return tokenInfo;
        }),
    );
...
const getCtokenInfo = async (
    ...
    const supplyApy = BigNumber.parseValue(
        (Math.pow(1 + supplyRatePerBlock.toNumber() / mantissa, network.blocksPerYear) - 1).noExponents(),
    );
    const borrowRatePerBlock = BigNumber.from(token.borrowRatePerBlock, 18);

    const borrowApy = BigNumber.parseValue(
        (Math.pow(1 + borrowRatePerBlock.toNumber() / mantissa, network.blocksPerYear) - 1).noExponents(),
    );
    ...
    const totalMaxSupplyApy = BigNumber.parseValue(
        (Math.max(+hndAPR.toString(), +veHndMaxAPR.toString()) + +supplyApy.toString()).noExponents(),
    );
    const totalMinSupplyApy = BigNumber.parseValue(
        (Math.max(+hndAPR.toString(), +veHndAPR.toString()) + +supplyApy.toString()).noExponents(),
    );
...
export const compareData = (oldMarkets: (CTokenInfo | null)[], newMarkets: CTokenInfo[]): void => {
    ...
    c = {
        name: 'HndSpeed',
        oldData: m && m.hndSpeed ? m.hndSpeed.toString() : '',
        newData: nm && nm.hndSpeed ? nm.hndSpeed.toString() : '',
    };
    compare.push(c);
    c = {
        name: 'Decimals',
