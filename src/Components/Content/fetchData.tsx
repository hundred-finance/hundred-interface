...
{ allMarkets: string[]; userAddress: string; comptrollerData: Comptroller; network: Network; marketsData: (CTokenInfo | null)[] | null | undefined; provider: any; hndPrice: number; gaugesData: GaugeV4[] }) : Promise<MarketDataType> => {
 ...
 const getTokenData = async(tokenData: any[], native: boolean, network: Network, provider: ethers.providers.Web3Provider, 
  ...
  const getCtokenInfo = async (token: Token, network: Network, hndPrice: number, rewardTokenPrice: number, blockNum: number, gauges: GaugeV4[] | undefined) : Promise<CTokenInfo> => {
      ...
      const supplyApy = BigNumber.parseValue((Math.pow((1 + +supplyRatePerBlock.toString() / mantissa), network.blocksPerYear) - 1).noExponents())
      const borrowRatePerBlock = BigNumber.from(token.borrowRatePerBlock, 18)

      const borrowApy = BigNumber.parseValue((Math.pow((1 + +borrowRatePerBlock.toString() / mantissa), network.blocksPerYear) - 1).noExponents())
