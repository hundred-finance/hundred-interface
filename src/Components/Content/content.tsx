...
const Content: React.FC<Props> = (props: Props) => {
                    ...
                    if (market) {
                        if (cToken && market.underlying.symbol === cToken.underlying.symbol && spinner) {
                            ...
                            if (market.backstop && m.backstop) {
                                m.backstopDepositSpinner =
                                    spinner === 'deposit' ? false : market.backstopDepositSpinner;
                                m.backstopWithdrawSpinner =
                                    spinner === 'backstopWithdraw' ? false : market.backstopWithdrawSpinner;
                                m.backstopClaimSpinner =
                                    spinner === 'backstopClaim' ? false : market.backstopClaimSpinner;
                            }
                        } else {
        ...
        try {
            ...
            if (marketsRef.current)
                setUpdateHandle(
                    setTimeout(
                        handleUpdate,
                        (updateErrorCounterRef.current < 2
                            ? updateErrorCounterRef.current + 1
                            : updateErrorCounterRef.current) *
                            10000 +
                            10000,
                        market,
                        spinnerUpdate,
                ...
                if (updateErrorCounterRef.current < 2)
                    setUpdateHandle(
                        setTimeout(handleUpdate, (updateErrorCounterRef.current + 1) * 1000, market, spinnerUpdate),
                    );
                    ...
                    props.toastError(
                        `${err?.message.replace('.', '')} on Page Load\n${
                            err?.data?.message
    ...
    const getMaxRepayAmount = async (market: CTokenInfo): Promise<BigNumber> => {
        ...
        const amount = BigNumber.parseValueSafe(
            market.borrowBalanceInTokenUnit.mulSafe(maxRepayFactor).toString(),
                ...
                try {
                    ...
                    if (market.underlying.address && network.current.backstopMasterChef) {
                        ...
                        const tx = await contract.approve(
                            network.current.backstopMasterChef.address,
                            MaxUint256._value,
                        );
                        if (spinner.current) spinner.current(false);
                        console.log(tx);
        ...
        if (marketsRef.current && network.current && network.current.backstopMasterChef) {
            ...
            if (market && market.backstop && provider.current && network.current.backstopMasterChef) {
                try {
                    ...
                    const signer = provider.current.getSigner();
                    const backstopAbi =
                        network.current.backstopMasterChef.version === MasterChefVersion.v1
                            ? BACKSTOP_MASTERCHEF_ABI
                            : BACKSTOP_MASTERCHEF_ABI_V2;
                    const backstop = new ethers.Contract(
                        network.current.backstopMasterChef.address,
                        backstopAbi,
                        signer,
                    );
        ...
        if (marketsRef.current && network.current && network.current.backstopMasterChef) {
            ...
            if (market && market.backstop && provider.current && network.current.backstopMasterChef) {
                try {
                    ...
                    const tx = await backstop.withdrawAndHarvest(
                        market.backstop.pool.poolId,
                        tokenAmount,
                        userAddress.current,
                    );
                    ...
                    if (market) {
                        ...
                        if (
                            ...
                            selectedMarketRef.current.underlying.symbol === market.underlying.symbol
                        )
    ...
    return (
        <div className="content">
            ...
            <SupplyMarketDialog
                ...
                gaugeV4={gaugesV4Data?.find(
                    (g) =>
                        g?.generalData.lpToken.toLowerCase() === selectedMarketRef.current?.pTokenAddress.toLowerCase(),
                )}
