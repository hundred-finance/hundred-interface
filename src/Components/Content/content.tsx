            ...
            if (market && provider.current) {
                try {
                    ...
                    if (market.isNativeToken) {
                        const maxiContract = new ethers.Contract(market.pTokenAddress, MAXIMILLION_ABI, signer);
                        const tx = await maxiContract.repayBehalfExplicit(signer._address, market.pTokenAddress);
                        console.log(tx);
                        if (spinner.current) spinner.current(false);
                        const receipt = await tx.wait();
                        console.log(receipt);
