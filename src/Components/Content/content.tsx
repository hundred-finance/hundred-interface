...
import { MAXIMILLION_ABI, CTOKEN_ABI, TOKEN_ABI, CETHER_ABI, BACKSTOP_MASTERCHEF_ABI, BACKSTOP_MASTERCHEF_ABI_V2 } from '../../abi';
...
const Content: React.FC<Props> = (props: Props) => {
  ...
  useEffect(() => {
    ...
    updateEarnedRef.current = props.updateEarned;
    if (props.updateEarned) callUpdate();
  }, [props.updateEarned]);
    ...
    if (provider.current && network.current && userAddress.current) {
      ...
      if (provider.current && comptrollerDataRef.current) {
        ...
        const markets = await fetchData({
          allMarkets: comptrollerDataRef.current.allMarkets,
          userAddress: userAddress.current,
          comptrollerData: comptrollerDataRef.current,
          network: network.current,
          marketsData: marketsRef.current,
          provider: provider.current,
          hndPrice: hndPriceRef.current,
          gaugesData: gauges,
        });
    ...
    try {
      ...
      if (marketsRef.current)
        setUpdateHandle(
          setTimeout(
            handleUpdate,
            (updateErrorCounterRef.current < 2 ? updateErrorCounterRef.current + 1 : updateErrorCounterRef.current) * 10000 + 10000,
            market,
            spinnerUpdate,
          ),
        );
      else {
        if (updateErrorCounterRef.current < 2) setUpdateHandle(setTimeout(handleUpdate, (updateErrorCounterRef.current + 1) * 1000, market, spinnerUpdate));
        else if (updateErrorCounterRef.current === 3) setUpdateHandle(setTimeout(handleUpdate, 5000, market, spinnerUpdate));
        ...
        } else setUpdateHandle(setTimeout(handleUpdate, 10000, market, spinnerUpdate));
      }
    ...
    if (market.isNativeToken && props.provider) {
      ...
      if (func === 'repay' && provider.current) {
        ...
        const balance = market.underlying.walletBalance.gt(BigNumber.from('0'))
          ? market.underlying.walletBalance.subSafe(gasRemainder)
          : market.underlying.walletBalance;
    ...
    if (marketsRef.current) {
      ...
      if (market && provider.current && comptrollerDataRef.current && network.current && userAddress.current) {
        try {
          ...
          if (market) await handleUpdate(market, 'spinner');
        }
      }
    }
  };
      ...
      if (market && provider.current && network.current && userAddress.current) {
        try {
          ...
          if (market.underlying.address) {
            ...
            if (selectedMarketRef.current) borrowDialog ? (selectedMarketRef.current.repaySpinner = true) : (selectedMarketRef.current.supplySpinner = true);
    ...
    if (marketsRef.current) {
      ...
      if (market && provider.current) {
        try {
          ...
          const tokenAmount = market.isNativeToken ? { value: value._value } : value._value;
          if (selectedMarketRef.current) selectedMarketRef.current.supplySpinner = true;
          ...
          const tx = await ctoken.mint(tokenAmount);
    ...
    if (marketsRef.current) {
      ...
      if (market && provider.current) {
        try {
          ...
          if (selectedMarketRef.current) selectedMarketRef.current.withdrawSpinner = true;
    ...
    if (marketsRef.current) {
      ...
      if (market && provider.current) {
        try {
          setCompleted(false);
          const value = BigNumber.parseValueSafe(amount, market.underlying.decimals);
          if (selectedMarketRef.current) selectedMarketRef.current.borrowSpinner = true;
      ...
      if (market && market.backstop && provider.current && network.current && userAddress.current) {
        try {
          ...
          if (market.underlying.address && network.current.backstopMasterChef) {
            ...
            if (selectedMarketRef.current && selectedMarketRef.current.backstop) selectedMarketRef.current.backstopDepositSpinner = true;
    ...
    if (marketsRef.current && network.current && network.current.backstopMasterChef) {
      ...
      if (market && market.backstop && provider.current && network.current.backstopMasterChef) {
        try {
          setCompleted(false);
          const value = BigNumber.parseValueSafe(amount, market.underlying.decimals);
          const tokenAmount = value._value;
          if (selectedMarketRef.current && selectedMarketRef.current.backstop) selectedMarketRef.current.backstopDepositSpinner = true;
          market.backstopDepositSpinner = true;
          ...
          const tx = await backstop.deposit(market.backstop.pool.poolId, tokenAmount, userAddress.current);
    ...
    if (marketsRef.current && network.current && network.current.backstopMasterChef) {
      ...
      if (market && market.backstop && provider.current && network.current.backstopMasterChef) {
        try {
          ...
          if (selectedMarketRef.current && selectedMarketRef.current.backstop) selectedMarketRef.current.backstopWithdrawSpinner = true;
          const value = BigNumber.parseValueSafe(amount, market.backstop.decimals);
          const tokenAmount = value._value;
          const signer = provider.current.getSigner();
          const backstopAbi = network.current.backstopMasterChef.version === MasterChefVersion.v1 ? BACKSTOP_MASTERCHEF_ABI : BACKSTOP_MASTERCHEF_ABI_V2;
          const backstop = new ethers.Contract(network.current.backstopMasterChef.address, backstopAbi, signer);
          const tx = await backstop.withdrawAndHarvest(market.backstop.pool.poolId, tokenAmount, userAddress.current);
    ...
    if (marketsRef.current && network.current && network.current.backstopMasterChef) {
      ...
      if (market && market.backstop && provider.current && network.current.backstopMasterChef) {
        try {
          ...
          if (selectedMarketRef.current && selectedMarketRef.current.backstop) selectedMarketRef.current.backstopClaimSpinner = true;
          const signer = provider.current.getSigner();
          const backstopAbi = network.current.backstopMasterChef.version === MasterChefVersion.v1 ? BACKSTOP_MASTERCHEF_ABI : BACKSTOP_MASTERCHEF_ABI_V2;
          const backstop = new ethers.Contract(network.current.backstopMasterChef.address, backstopAbi, signer);
    ...
    if (marketsRef.current) {
      ...
      if (market && provider.current) {
        try {
          ...
          if (selectedMarketRef.current) selectedMarketRef.current.stakeSpinner = true;
    ...
    if (marketsRef.current) {
      ...
      if (market && provider.current) {
        try {
          ...
          if (selectedMarketRef.current) selectedMarketRef.current.unstakeSpinner = true;
    ...
    if (marketsRef.current) {
      ...
      if (market && provider.current) {
        try {
          ...
          if (selectedMarketRef.current) selectedMarketRef.current.mintSpinner = true;
  ...
  return (
    <div className="content">
      ...
      <EnterMarketDialog
        ...
        handleEnterMarket={handleEnterMarket}
        handleExitMarket={handleExitMarket}
      />
      <SupplyMarketDialog
        completed={completed}
      ...
      <BorrowMarketDialog
        ...
        getMaxRepayAmount={getMaxRepayAmount}
        spinnerVisible={props.spinnerVisible}
      />
    </div>
  );
