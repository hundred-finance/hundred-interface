...
export class xDefiConnector extends AbstractConnector {
  ...
  public async activate(): Promise<any> {
    ...
    if (!account) {
      ...
      account = await window.xfi.ethereum.enable().then((sendReturn: any) => sendReturn && parseSendReturn(sendReturn)[0])
    }
  ...
  public async getAccount(): Promise<null | string> {
    ...
    let account
    try {
      account = await (window.xfi.ethereum.send as Send)('eth_accounts').then(sendReturn => parseSendReturn(sendReturn)[0])
    } catch {
