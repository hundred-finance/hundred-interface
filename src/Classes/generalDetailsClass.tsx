...
export const getGeneralDetails = (
    ...
    marketsData.map((market) => {
        if (market) {
            const gauge = gauges.find(
                (g) => g.generalData.lpToken.toLowerCase() === market.pTokenAddress.toLowerCase(),
            );
            const stake = gauge
                ? (+gauge.generalData.totalStake * +market.exchangeRate * +market.underlying.price) /
                  10 ** market.underlying.decimals
                : 0;
            const userStake = gauge
                ? (+gauge.userStakedTokenBalance * +market.exchangeRate * +market.underlying.price) /
                  10 ** market.underlying.decimals
                : 0;

            totalSupplyBalance = BigNumber.parseValue(
                (+totalSupplyBalance.toString() + +market.supplyBalance.toString()).noExponents(),
            );
            totalBorrowBalance = BigNumber.parseValue(
                (+totalBorrowBalance.toString() + +market.borrowBalance.toString()).noExponents(),
            );
            totalStakedBalance = BigNumber.parseValue((+totalStakedBalance.toString() + userStake).noExponents());

            allMarketsTotalStakeBalance = BigNumber.parseValue(
                (+allMarketsTotalStakeBalance.toString() + stake).noExponents(),
            );

            if (+market?.marketTotalSupply?.toString() > 0) {
                allMarketsTotalSupplyBalance = BigNumber.parseValue(
                    (+allMarketsTotalSupplyBalance.toString() + +market.marketTotalSupply.toString()).noExponents(),
                );
            }
            ...
            totalBorrowLimit = totalBorrowLimit.addSafe(
                market.isEnterMarket ? market.supplyBalance.mul(market.collateralFactor) : BigNumber.from('0'),
            );

            yearSupplyInterest = BigNumber.parseValue(
                (
                    +yearSupplyInterest.toString() +
                    +market.supplyBalance.toString() * (+market.supplyApy.toString() + +market.hndAPR.toString())
                ).noExponents(),
            );
            yearStakeInterest = BigNumber.parseValue(
                (+yearStakeInterest.toString() + userStake * +market.veHndAPR.toString()).noExponents(),
            );
            yearBorrowInterest = BigNumber.parseValue(
                (
                    +yearBorrowInterest.toString() +
                    +market.borrowBalance.toString() * +market.borrowApy.toString()
                ).noExponents(),
            );
            yearSupplyHndRewards = BigNumber.parseValue(
                (+yearSupplyHndRewards.toString() + +market.hndAPR.toString()).noExponents(),
            );
    ...
    const tempNetApy =
        +totalSupplyBalance.toString() + +totalStakedBalance.toString() - +totalBorrowBalance.toString() > 0
            ? (+yearSupplyInterest.toString() + +yearStakeInterest.toString() - +yearBorrowInterest.toString()) /
              (+totalSupplyBalance.toString() + +totalStakedBalance.toString() - +totalBorrowBalance.toString())
            : 0;
    ...
    return new GeneralDetailsData(
        ...
        yearBorrowInterest,
        netApy,
        +totalSupplyBalance.toString() > 0
            ? BigNumber.parseValue(
                  ((+yearSupplyHndRewards.toString() / +totalSupplyBalance.toString()) * 100).noExponents(),
              )
            : BigNumber.from(0),
        +totalBorrowBalance.toString() > 0
            ? BigNumber.parseValue((+yearBorrowHndRewards.toString() / +totalBorrowBalance.toString()).noExponents())
            : BigNumber.from(0),
        totalLiquidity,
