...
export class Backstop {
    ...
    constructor(
        ...
        const tvl =
            +underlyingBalance.toString() * +tokenPrice.toString() + +fetchPrice.toString() * +ethBalance.toString();
        ...
        this.sharePrice =
            totalSupply.toNumeral() > 0
                ? BigNumber.parseValue((tvl / +totalSupply.toString()).noExponents())
                : BigNumber.from('0');
        this.symbol = symbol;
        ...
        this.userEthBalance = BigNumber.parseValue(
            ((+ethBalance.toString() * +userBalance.toString()) / +totalSupply.toString()).noExponents(),
        );
        this.apr =
            totalAllocPoint > 0 && hndPrice > 0
                ? BigNumber.parseValue(
                      (
                          (((+hundredPerSecond.toString() * (60 * 60 * 24 * 365) * poolInfo.allocPoint) /
                              totalAllocPoint) *
                              hndPrice) /
                          tvl
                      ).noExponents(),
...
export class BackstopV2 {
    ...
    constructor(backstop: BackstopTypeV2, tokenDecimals: number, tokenPrice: BigNumber, hndPrice: number) {
        ...
        backstop.collaterals.forEach(
            (c) =>
                (tvl +=
                    ((+c.balance / 10 ** c.decimals) * +c.fetchPrice) / 10 ** tokenDecimals / 10 ** (18 - c.decimals)),
        );
        ...
        this.sharePrice =
            this.totalSupply.toNumeral() > 0
                ? BigNumber.parseValue((tvl / +this.totalSupply.toString()).noExponents())
                : BigNumber.from('0');
        this.symbol = backstop.symbol;
        this.allowance = BigNumber.from(backstop.allowance, backstop.decimals);
        this.tvl = BigNumber.parseValue(tvl.noExponents());
        this.apr =
            backstop.totalAllocPoint > 0 && hndPrice > 0
                ? BigNumber.parseValue(
                      (
                          (((+this.hundedPerSecond.toString() * (60 * 60 * 24 * 365) * backstop.poolInfo.allocPoint) /
                              backstop.totalAllocPoint) *
                              hndPrice) /
                          tvl
                      ).noExponents(),
                  )
