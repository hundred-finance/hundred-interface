    ...
    if (backstop) {
        ...
        if (backstopData && backstopData.length === 2 * poolLength) {
            ...
            if (network.backstopMasterChef?.version === MasterChefVersion.v2) {
                ...
                const collateralsCount = network.backstopMasterChef.collaterals
                    ? network.backstopMasterChef.collaterals
                    : 0;
                ...
                backstopPools.forEach((x) => {
                    ...
                    for (let i = 0; i < collateralsCount; i++) collateralsData.push(collaterals[i]);
...
// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
export const getComptrollerData2 = async (
    provider: any,
    userAddress: string,
    network: Network,
): Promise<Comptroller2> => {
    const ethcallProvider = new Provider();
    await ethcallProvider.init(provider);
    const ethcallComptroller = new Contract(network.unitrollerAddress, COMPTROLLER_ABI);

    const [oracleAddress, allMarkets, enteredMarkets] = await ethcallProvider.all([
        ethcallComptroller.oracle(),
        ethcallComptroller.getAllMarkets(),
        ethcallComptroller.getAssetsIn(userAddress),
    ]);
    const comptroller = new ethers.Contract(network.unitrollerAddress, COMPTROLLER_ABI, provider);
